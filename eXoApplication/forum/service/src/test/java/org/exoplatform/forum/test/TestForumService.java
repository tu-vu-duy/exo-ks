...
public class TestForumService extends BaseForumTestCase{
  ...
  public void testCategory() throws Exception {  
    ...
    // test removeCategory
    cat = forumService_.removeCategory(catId);
...
//	move topic from forum to forum 1
  ...
  public void testPost() throws Exception {
		...
		//get ListPost
		JCRPageList<Post> pagePosts = forumService_.getPosts(categoryId, forumId, topicId, "", "", "", "root");
		assertEquals(pagePosts.getAvailable(), posts.size() + 1);// size = 26 (first post and new postList)
    List<Topic> page1 = pagePosts.getPage(1) ;
    assertEquals(page1.size(), 10);  
    List<Topic> page3 = pagePosts.getPage(3) ;
    assertEquals(page3.size(), 6);
    // getPost by Ip
    JCRPageList<Post> pageIpPosts = forumService_.getListPostsByIP("192.168.1.11", null);
  ...
  public void testTag() throws Exception{
  	//  set Data
		...
		//	Test get tag
		tag = forumService_.getTag(tag.getId());
		assertNotNull(tag);
