...
public class UIAttachFileForm extends UIForm implements UIPopupComponent {
  ...
  static  public class SaveActionListener extends EventListener<UIAttachFileForm> {
    public void execute(Event<UIAttachFileForm> event) throws Exception {
      ...
      List<WikiResource> files = new ArrayList<WikiResource>() ;
      byte[] imageBytes;
      int i = 0 ;
      ...
      while(i++ < uiForm.maxField) {
        ...
        if(uploadResource != null) {
          ...
          WikiResource attachfile = new WikiResource(uploadResource.getMimeType(), "UTF-8", imageBytes) ;
      ...
      if(files.isEmpty()){
        ...
        UIWikiPortlet uiWikiPortlet = uiForm.getAncestorOfType(UIWikiPortlet.class) ;
        UIPopupContainer uiPopupContainer = uiWikiPortlet.getChild(UIPopupContainer.class) ;
        UIWikiPageContentArea uiPageContentArea = uiWikiPortlet.findFirstComponentOfType(UIWikiPageContentArea.class) ;
        for(WikiResource file :  files){
          uiPageContentArea.addToUploadFileList(file) ;  
        }
  ...
  static  public class CancelActionListener extends EventListener<UIAttachFileForm> {
    public void execute(Event<UIAttachFileForm> event) throws Exception {
      ...
      UIPopupContainer uiPopupContainer = uiFileForm.getAncestorOfType(UIPopupContainer.class) ;
      uiPopupContainer.deActivate() ;
      event.getRequestContext().addUIComponentToUpdateByAjax(uiPopupContainer) ;
    }
  }
